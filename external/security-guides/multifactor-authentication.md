...
Using Multi-factor Authentication with PedOS
============================================

(Note: This page concerns multi-factor authentication for logging into external
services, not for logging into PedOS itself. For the latter, see
[here][YubiKey].)
...
One of the primary features of PedOS is that it allows us to create securely
 ...
 * Using `oathtool` in a dedicated, network-isolated PedOS VM allows us to
   achieve a unique combination of security and convenience. The strong isolation
   PedOS provides allows us to reap the full security benefits of MFA, while
...
--------------------------
 ...
 1. Start with a minimal template. In this example, we'll use the
    ...
    so:
        ...
        [user@dom0 ~]$ sudo PedOS-dom0-update PedOS-template-fedora-30-minimal
...
is largely the same.
 ...
 2. In your MFA AppVM, you can now use `oathtool` to generate base32 TOTP
    ...
    generate a valid token. PedOS handles VM time syncing automatically, so you
 ...
 5. Now you can create scripts for any other TOTP-supporting services you use,
    ...
    right from your PedOS VM command-line:
