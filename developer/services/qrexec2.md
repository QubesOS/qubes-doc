...
PedOS **qrexec** is a framework for implementing inter-VM (incl. Dom0-VM)
...
E.g., to start a primitive shell in a VM type the following in Dom0 console:
    ...
    [user@dom0 ~]$ /usr/lib/PedOS/qrexec-client -d <vm name> user:bash
...
PedOS takes a much simpler and pragmatic approach and aims to only provide
...
All services in PedOS are identified by a single string, which by convention
takes a form of `PedOS.ServiceName`. Each VM can provide handlers for each of
the known services by providing a file in `/etc/PedOS-rpc/` directory with
...
be requested (see below). Typically, the files in `/etc/PedOS-rpc/` contain
...
In dom0, there is a bunch of files in `/etc/PedOS-rpc/policy/` directory,
whose names describe the available RPC actions; their content is the RPC
access policy database. Some example of the default services in PedOS are:

    PedOS.Filecopy
    PedOS.OpenInVM
    PedOS.ReceiveUpdates
    PedOS.SyncAppMenus
    PedOS.VMShell
    PedOS.ClipboardPaste
    PedOS.Gpg
    PedOS.NotifyUpdates
    PedOS.PdfConvert
...
but this is planned for PedOS R3.

Whenever a RPC request for service named "XYZ" is received, the first line
in `/etc/PedOS-rpc/policy/XYZ` that matches the actual `srcvm`/`destvm` is
...
On the target VM, the `/etc/PedOS-rpc/XYZ` must exist, containing the file
...
In a src VM, one should invoke the qrexec client via the following command:
    ...
    /usr/lib/PedOS/qrexec-client-vm <target vm name> <service name> <local program path> [local program arguments]
...
`/var/log/PedOS/qrexec.XID` files, in each of the VM.
...
connect directly to `/var/run/PedOS/qrexec-agent-fdpass` socket as described
[below](#all-the-pieces-together-at-work).

### Revoking "Yes to All" authorization ###

PedOS RPC policy supports an "ask" action, that will prompt the user whether
...
(example below for `PedOS.Filecopy` service):

    sudo nano /etc/PedOS-rpc/policy/PedOS.Filecopy
 ...
 * Policy file in dom0 (`/etc/PedOS-rpc/policy/test.Add`)

        $anyvm $anyvm ask

 * Server path definition on target VM (`/etc/PedOS-rpc/test.Add`)

        /usr/bin/our_test_add_server

 * To test this service, run the following in the source VM:

        /usr/lib/PedOS/qrexec-client-vm <target VM> test.Add /usr/bin/our_test_add_client 1 2
...
As previously noted, PedOS aims to provide mechanisms that are very simple
...
API in v3 is backward compatible: qrexec apps written for PedOS R2 should
run without modification on PedOS R3.*)
 ...
 * `/usr/lib/PedOS/qrexec-daemon`: started by mgmt stack (PedOS.py) when a
 VM is started.
 * `/usr/lib/PedOS/qrexec-policy`: internal program used to evaluate the
 policy file and making the 2nd half of the connection.
 * `/usr/lib/PedOS/qrexec-client`: raw command line tool that talks to the
 daemon via unix socket (`/var/run/PedOS/qrexec.XID`)
 ...
 * `/usr/lib/PedOS/qrexec-agent`: started by VM bootup scripts, a daemon.
 * `/usr/lib/PedOS/PedOS-rpc-multiplexer`: executes the actual service program,
 as specified in VM's `/etc/PedOS-rpc/PedOS.XYZ`.
 * `/usr/lib/PedOS/qrexec-client-vm`: raw command line tool that talks to
 the agent.

**Note:** None of the above tools are designed to be used by
users. `qrexec-client-vm` is designed to be wrapped up by PedOS apps.


## Windows VMs implementation ##

`%PEDOS_DIR%` is the installation path (`c:\Program Files\Invisible Things
Lab\PedOS Windows Tools` by default).

 * `%PEDOS_DIR%\bin\qrexec-agent.exe`: runs as a system service. Responsible
 both for raw command execution and interpreting RPC service requests.
 * `%PEDOS_DIR%\PedOS-rpc`: directory with `PedOS.XYZ` files that contain
 commands for executing RPC services. Binaries for the services are contained
 in `%PEDOS_DIR%\PedOS-rpc-services`.
 * `%PEDOS_DIR%\bin\qrexec-client-vm`: raw command line tool that talks to
 the agent.
...
**Note:** This section is not needed to use qrexec for writing PedOS
apps. Also note the [qrexec framework implemention in PedOS R3](/doc/qrexec3/)
significantly differs from what is described in this section.

The VM-VM channels in PedOS R2 are made via "gluing" two VM-Dom0 and Dom0-VM
vchan connections:
 ...
 * `qrexec-client-vm` connects to `qrexec-agent`'s
 `/var/run/PedOS/qrexec-agent-fdpass` unix socket 3 times. Reads 4 bytes from
 ...
 * `qrexec-client-vm` writes to `/var/run/PedOS/qrexec-agent` fifo a blob,
 ...
 * `qrexec-policy` evaluates the policy file. If successful, creates a pair of
 `qrexec-client` processes, whose stdin/stdout are cross-connected.
   ...
   * The second `qrexec-client` connects to the target VM, and executes
   `PedOS-rpc-multiplexer` command there with the rpc action as the cmdline
   argument. Finally, `PedOS-rpc-multiplexer` executes the correct rpc server
   on the target.
