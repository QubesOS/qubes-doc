...
In the sys-net VM’s Terminal, the first step is to to define an ntables
...
steps 2 and 3 described above, for each interface.
...
used in the previous steps in ``/rw/config/rc.local`` so they get set on
sys-net start-up:
