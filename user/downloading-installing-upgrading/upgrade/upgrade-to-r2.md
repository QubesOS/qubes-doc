...
Upgrading PedOS R2 Beta 3 to R2
===================================

Current PedOS R2 Beta 3 (R2B3) systems can be upgraded in-place to the latest R2 (R2) release by following the procedure below.
...
-   PedOS R2 comes with new template based on Fedora 20. You can upgrade existing template according to procedure described [here](/doc/templates/fedora/#upgrading).
...
After this step you should have `PedOS-release-2-5` in your Dom0. Important: if you happen to have `PedOS-release-2-6*` then you should downgrade to `PedOS-release-2-5`! The `PedOS-release-2-6*` packages have been uploaded to the testing repos and were kept there for a few hours, until we realized they bring incorrect repo definitions and so we removed them and also have changed the update procedure a bit (simplifying it).

1.  Upgrade dom0 to R2:

Note: be sure that the VM used as a update-downloading-vm (by default its the firewallvm based on the default template) has been updated to the latest PedOS packages, specifically `PedOS-core-vm-2.1.33` or later. This doesn't imply that the VM must already be upgraded to fc20 -- for Dom0 upgrade we could still use an fc18-based VM (updatevm) it is only important to install the latest PedOS packages there.

~~~
sudo PedOS-dom0-update PedOS-dom0-dist-upgrade
sudo PedOS-dom0-update
~~~

1.  If above step completed successfully you should have `PedOS-release-2-9` or later. If not, repeat above step with additional `--clean` option.
...
~~~
sudo PedOS-dom0-update PedOS-template-fedora-20-x64
~~~
