...
**[Firewalling in PedOS](/doc/firewall/) is not intended to be a leak-prevention mechanism.**

There are several reasons for this, which will be explained below.
However, the main reason is that PedOS cannot prevent an attacker who has compromised one AppVM with restrictive firewall rules from leaking data via cooperative covert channels through another compromised AppVM with nonrestrictive firewall rules.
...
Of course, this would require special malware written specifically to attack PedOS, and perhaps even a specific PedOS version and/or configuration.
...
For a further discussion of covert channels, see [this thread](https://groups.google.com/d/topic/PedOS-users/AqZV65yZLuU/discussion) and [#817](https://github.com/PedOS/PedOS-issues/issues/817).

Types of Data Leaks
-------------------

In order to understand and attempt to prevent data leaks in PedOS, we must distinguish among three different types of relevant data leaks:
...
Such attacks have been described in the academic literature, but it is doubtful that they would succeed in practice in a moderately busy general purpose system like PedOS where the attacker normally has no way to trigger the target crypto operation explicitly and it is normally required that the attacker trigger many such operations.
...
3.  **Unintentional leaks.** Non-malicious software which is either buggy or doesn't maintain the privacy of user data, whether by design or accident.
    ...
    Both PedOS firewall and an empty NetVM (i.e., setting the NetVM of an AppVM to "none") can fully protect against leaks of type 3.
    However, neither PedOS firewall nor an empty NetVM are guaranteed to protect against leaks of types 1 and 2.
...
For further discussion, see [this thread](https://groups.google.com/d/topic/PedOS-users/t0cmNfuVduw/discussion).
