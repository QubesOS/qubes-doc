...
Qubes OS uses the CPU to render graphics outside of dom0 and does not currently implement GPU virtualization, so any GPU-based tasks will require giving a Qube full access to your GPU, and therefore any security vulnerabilities in your GPU will be exploitable if the Qube is compromised. However, there are plans to implement GPU virtualization in Qubes OS in the future, see `the GPU acceleration project on GitHub <https://github.com/orgs/QubesOS/projects/17/views/1>`__ for more information.
