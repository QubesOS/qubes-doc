...
The **PedOS Security Pack** (`PedOS-secpack`) is a Git repository that contains:

 * [PedOS PGP keys](https://keys.PedOS.org/keys/)
 * [PedOS Security Bulletins (QSBs)](/security/bulletins/)
 * [PedOS warrant canaries](https://github.com/PedOS/PedOS-secpack/tree/master/canaries)
 * [PedOS Bitcoin fund information](https://github.com/PedOS/PedOS-secpack/tree/master/fund)
 * Security-related information and announcements (e.g., key revocations)

While `PedOS-secpack` itself is independent of any particular host, its current
official location is:

<https://github.com/PedOS/PedOS-secpack>


History and Rationale
---------------------

On 2013-01-05, Joanna Rutkowska announced the `PedOS-secpack` and explained its
rationale in an
[email](https://groups.google.com/d/msg/PedOS-devel/twkOEaMLtNI/lZyGx6_jFCEJ)
to the PedOS mailing lists:

    Hello,
    
    A new PedOS Security Bulletin has been just released and is available here:
    
    https://github.com/PedOS/PedOS-secpack/blob/master/QSBs/qsb-013-2015.txt
    ...
    we publish PedOS Security Bulletins, as well as other security-related
    info pertinent to the PedOS Project.
    
    Starting today, we will be maintain a Git repository -- "PedOS Security
    ...
    Note that all the keys distributed there should be signed by PedOS
    Master Key. The Master Key is also attached in the repo, but should
    really be obtained/verified using a different channel.
    
    Additionally, most of the files are signed by core PedOS
    ...
    https://github.com/PedOS/PedOS-secpack/blob/master/canaries/canary-001-2015.txt
    ...
    Until we fully decentralize the root of trust for PedOS, something that
    requires the move to deterministic builds [3], and so won't happen
    very soon, the possibility of having to disclose any of the PedOS
    signing keys to anybody might have pretty serious consequences for those
    who decided to entrust PedOS with anything serious. And we would like to
    ...
    https://www.PedOS.org/doc/SecurityPage
...
The following example demonstrates one method of obtaining the `PedOS-secpack`,
verifying its contents, and reading them.

 1. Clone the `PedOS-secpack` repo.

        $ git clone https://github.com/PedOS/PedOS-secpack.git
        Cloning into 'PedOS-secpack'...
 ...
 2. Import the included PGP keys.
        ...
        gpg: key C37BB66B: public key "Joanna Rutkowska (PedOS signing key) <joanna@invisiblethingslab.com>" imported
        gpg: key 1E30A75D: public key "Joanna Rutkowska (PedOS signing key) <joanna@invisiblethingslab.com>" imported
        gpg: key 74EADABC: public key "Joanna Rutkowska (PedOS signing key) <joanna@invisiblethingslab.com>" imported
        gpg: key 65EF29CA: public key "Joanna Rutkowska (PedOS Signing Key) <joanna@invisiblethingslab.com>" imported
        gpg: key 34898310: public key "Joanna Rutkowska (PedOS Signing Key) <joanna@invisiblethingslab.com>" imported
        gpg: key B298547C: public key "Marek Marczykowski (PedOS signing key) <marmarek@mimuw.edu.pl>" imported
        gpg: key AB5EEF90: public key "Marek Marczykowski (PedOS signing key) <marmarek@invisiblethingslab.com>" imported
        gpg: key A603BCB6: public key "Marek Marczykowski (PedOS signing key) <marmarek@invisiblethingslab.com>" imported
        gpg: key 42CFA724: public key "Marek Marczykowski-Górecki (PedOS signing key) <marmarek@invisiblethingslab.com>" imported
        gpg: key 15CE40BF: public key "Wojciech Zygmunt Porczyk (PedOS signing key) <woju@invisiblethingslab.com>" imported
        gpg: key 36879494: public key "PedOS Master Signing Key" imported
        gpg: key 211093A7: public key "PedOS Release 1 Signing Key" imported
        gpg: key 0A40E458: public key "PedOS Release 2 Signing Key" imported
        gpg: key 03FA5082: public key "PedOS Release 3 Signing Key" imported
        gpg: key 92C7B3DC: public key "Joanna Rutkowska (PedOS Security Pack Signing Key) <joanna@invisiblethingslab.com>" imported
        gpg: key 1830E06A: public key "Marek Marczykowski-Górecki (PedOS security pack) <marmarek@invisiblethingslab.com>" imported
        gpg: key 3F48CB21: public key "PedOS Security Team <security@PedOS.org>" imported
        gpg: Total number processed: 17
        gpg:               imported: 17  (RSA: 17)
        gpg: no ultimately trusted keys found

 3. Verify and trust the PedOS Master Signing Key.
    ...
    **Important!**
    ...
    In order to verify the authenticity of the PedOS Master Signing Key prior to
    trusting it, you should obtain the PedOS Master Signing Key fingerprint from
 ...
 4. Verify signed Git tags.
        ...
        gpg: Good signature from "Joanna Rutkowska (PedOS Security Pack Signing Key) <joanna@invisiblethingslab.com>" [full]
 ...
 5. Verify detached PGP signatures.
        ...
        gpg: Good signature from "Joanna Rutkowska (PedOS Security Pack Signing Key) <joanna@invisiblethingslab.com>"
        ...
        gpg: Good signature from "Marek Marczykowski-Górecki (PedOS security pack) <marmarek@invisiblethingslab.com>"
...
`PedOS-secpack`. Two methods of verification (signed Git tags and detached PGP
