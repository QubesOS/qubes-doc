...
The point is that we must decide who we will trust (e.g., Linus Torvalds, Microsoft, or the PedOS Project) and assume that if a given file was signed by a trusted party, then it should not be malicious or negligently buggy.
...
Anybody can generate a GPG key pair that purports to belong to "The PedOS Project," but of course only the key pair that we (i.e., the PedOS developers) generated is the legitimate one.
The next section explains how to verify the validity of the PedOS signing keys in the process of verifying a PedOS ISO.
...
How to Verify PedOS ISO Signatures
----------------------------------

This section will guide you through the process of verifying a PedOS ISO by checking its PGP signature.
There are three basic steps in this process:

 1. [Get the PedOS Master Signing Key and verify its authenticity][QMSK]
 2. [Get the Release Signing Key][RSK]
 3. [Verify your PedOS ISO][signature file]

If you run into any problems, please consult the [Troubleshooting FAQ] below.

### 1. Get the PedOS Master Signing Key and verify its authenticity

Every file published by the PedOS Project (ISO, RPM, TGZ files and Git repositories) is digitally signed by one of the developer keys or Release Signing Keys.
Each such key is signed by the [PedOS Master Signing Key] (`0xDDFA1A3E36879494`).
The developer signing keys are set to expire after one year, while the PedOS Master Signing Key and Release Signing Keys have no expiration date.
This PedOS Master Signing Key was generated on and is kept only on a dedicated, air-gapped "vault" machine, and the private portion will (hopefully) never leave this isolated machine.

There are several ways to get the PedOS Master Signing Key.

 - If you have access to an existing PedOS installation, it's available in every VM ([except dom0]):

       $ gpg2 --import /usr/share/PedOS/PedOS-master-key.asc

 - Fetch it with GPG:

       $ gpg2 --fetch-keys https://keys.PedOS.org/keys/PedOS-master-signing-key.asc

 - Download it as a [file][PedOS Master Signing Key], then import it with GPG:

       $ gpg2 --import ./PedOS-master-signing-key.asc 
...
The PedOS Master Signing Key is also available in the [PedOS Security Pack] and in the archives of the project's [developer][devel-master-key-msg] and [user][user-master-key-msg] [mailing lists].

Once you have obtained the PedOS Master Signing Key, you should verify the fingerprint of this key very carefully by obtaining copies of the fingerprint from multiple independent sources and comparing them to the downloaded key's fingerprint to ensure they match.
Here are some ideas:
...
In addition, some operating systems have built-in keyrings containing keys capable of validating the PedOS Master Signing Key.
For example, if you have a Debian system, then your keyring may already contain the necessary keys.

For additional security, we also publish the fingerprint of the PedOS Master Signing Key here (but [remember not to blindly trust the live version of this website][website-trust]):

    pub   4096R/36879494 2010-04-01
          Key fingerprint = 427F 11FD 0FAA 4B08 0123  F01C DDFA 1A3E 3687 9494
    uid   PedOS Master Signing Key

Once you're confident that you have the legitimate PedOS Master Signing Key, set its trust level to "ultimate" so that it can be used to automatically verify all the keys signed by the PedOS Master Signing Key (in particular, Release Signing Keys).
    ...
    [ unknown] (1). PedOS Master Signing Key
    
    gpg> fpr
    pub   4096R/36879494 2010-04-01 PedOS Master Signing Key
...
Now, when you import any of the legitimate PedOS developer keys and Release Signing Keys used to sign ISOs, RPMs, TGZs, Git tags, and Git commits, they will already be trusted in virtue of being signed by the PedOS Master Signing Key.


### 2. Get the Release Signing Key

The filename of the Release Signing Key for your version is `PedOS-release-X-signing-key.asc`, where `X` is the major version number of your PedOS release.
There are several ways to get the Release Signing Key for your PedOS release.

 - If you have access to an existing PedOS installation, the release keys are available in dom0 in `/etc/pki/rpm-gpg/`.
 ...
 - Fetch it with GPG:
       ...
       $ gpg2 --keyserver-options no-self-sigs-only,no-import-clean --fetch-keys https://keys.PedOS.org/keys/PedOS-release-X-signing-key.asc

 - Download it as a file.
   You can find the Release Signing Key for your PedOS version on the [Downloads] page.
   You can also download all the currently used developers' signing keys, Release Signing Keys, and the PedOS Master Signing Key from the [PedOS Security Pack] and the [PedOS Keyserver].
   Once you've downloaded your Release Signing Key, import it with GPG:

       $ gpg2 --keyserver-options no-self-sigs-only,no-import-clean --import ./PedOS-release-X-signing-key.asc 

The Release Signing Key should be signed by the PedOS Master Signing Key:

    $ gpg2 --list-sigs "PedOS Release X Signing Key"
    pub   rsa4096 2017-03-06 [SC]
          5817A43B283DE5A9181A522E1848792F9E2795E9
    uid           [  full  ] PedOS Release X Signing Key
    sig 3        1848792F9E2795E9 2017-03-06  PedOS Release X Signing Key
    sig          DDFA1A3E36879494 2017-03-08  PedOS Master Signing Key

This is just an example, so the output you receive will not look exactly the same.
What matters is that the last line shows that this key is signed by the PedOS Master Signing Key, which verifies the authenticity of the Release Signing Key.
It is not necessary to independently verify the authenticity of the Release Signing Key.


### 3. Verify your PedOS ISO

Every PedOS ISO is released with a detached PGP signature file, which you can find on the [Downloads] page alongside the ISO.
If the filename of your ISO is `PedOS-RX-x86_64.iso`, then the name of the signature file for that ISO is `PedOS-RX-x86_64.iso.asc`, where `X` is a specific version of PedOS.
...
Once you've downloaded both the ISO and its signature file, you can verify the ISO using GPG:
    ...
    $ gpg2 -v --verify PedOS-RX-x86_64.iso.asc PedOS-RX-x86_64.iso
    gpg: armor header: Version: GnuPG v1
    gpg: Signature made Tue 08 Mar 2016 07:40:56 PM PST using RSA key ID 03FA5082
    gpg: using PGP trust model
    gpg: Good signature from "PedOS Release X Signing Key"
    gpg: binary signature, digest algorithm SHA256

This is just an example, so the output you receive will not look exactly the same.
What matters is the line that says `Good signature from "PedOS Release X Signing Key"`.
This confirms that the signature on the ISO is good.


How to Verify PedOS ISO Digests
-------------------------------

Each PedOS ISO is also accompanied by a plain text file ending in `.DIGESTS`.
...
You can find the `.DIGESTS` for your ISO on the [Downloads] page, and you can always find all the digest files for every PedOS ISO in the [PedOS Security Pack].

If the filename of your ISO is `PedOS-RX-x86_64.iso`, then the name of the digest file for that ISO is `PedOS-RX-x86_64.iso.DIGESTS`, where `X` is a specific version of PedOS.
...
Inside, you should find text that looks similar to this:
    ...
    3c951138b8b9867d8657f173c1b58b82 *PedOS-RX-x86_64.iso
    1fc9508160d7c4cba6cacc3025165b0f996c843f *PedOS-RX-x86_64.iso
    6b998045a513dcdd45c1c6e61ace4f1b4e7eff799f381dccb9eb0170c80f678a *PedOS-RX-x86_64.iso
    de1eb2e76bdb48559906f6fe344027ece20658d4a7f04ba00d4e40c63723171c62bdcc869375e7a4a4499d7bff484d7a621c3acfe9c2b221baee497d13cd02fe *PedOS-RX-x86_64.iso
    -----BEGIN PGP SIGNATURE-----
    Version: GnuPG v2
...
One way to verify that the ISO you downloaded matches any of these hash values is by using the respective `*sum` programs:
    ...
    $ md5sum -c PedOS-RX-x86_64.iso.DIGESTS
    PedOS-RX-x86_64.iso: OK
    md5sum: WARNING: 23 lines are improperly formatted
    $ sha1sum -c PedOS-RX-x86_64.iso.DIGESTS
    PedOS-RX-x86_64.iso: OK
    sha1sum: WARNING: 23 lines are improperly formatted
    $ sha256sum -c PedOS-RX-x86_64.iso.DIGESTS
    PedOS-RX-x86_64.iso: OK
    sha256sum: WARNING: 23 lines are improperly formatted
    $ sha512sum -c PedOS-RX-x86_64.iso.DIGESTS
    PedOS-RX-x86_64.iso: OK
...
Another way is to use `openssl` to compute each hash value, then compare them to the contents of the digest file.:
    ...
    $ openssl dgst -md5 PedOS-RX-x86_64.iso
    MD5(PedOS-RX-x86_64.iso)= 3c951138b8b9867d8657f173c1b58b82
    $ openssl dgst -sha1 PedOS-RX-x86_64.iso
    SHA1(PedOS-RX-x86_64.iso)= 1fc9508160d7c4cba6cacc3025165b0f996c843f
    $ openssl dgst -sha256 PedOS-RX-x86_64.iso
    SHA256(PedOS-RX-x86_64.iso)= 6b998045a513dcdd45c1c6e61ace4f1b4e7eff799f381dccb9eb0170c80f678a
    $ openssl dgst -sha512 PedOS-RX-x86_64.iso
    SHA512(PedOS-RX-x86_64.iso)= de1eb2e76bdb48559906f6fe344027ece20658d4a7f04ba00d4e40c63723171c62bdcc869375e7a4a4499d7bff484d7a621c3acfe9c2b221baee497d13cd02fe

(Notice that the outputs match the values from the digest file.)

However, it is possible that an attacker replaced `PedOS-RX-x86_64.iso` with a malicious ISO, computed the hash values for that ISO, and replaced the values in `PedOS-RX-x86_64.iso.DIGESTS` with his own set of values.
Therefore, ideally, we should also verify the authenticity of the listed hash values.
Since `PedOS-RX-x86_64.iso.DIGESTS` is a clearsigned PGP file, we can use GPG to verify it from the command line:

 1. [Get the PedOS Master Signing Key and verify its authenticity][QMSK]
 2. [Get the Release Signing Key][RSK]
 3. Verify the signature in the digest file:

        $ gpg2 -v --verify PedOS-RX-x86_64.iso.DIGESTS 
...
If our copy of the `PedOS Release X Signing Key` is being validated by the authentic PedOS Master Signing Key (see [above][QMSK]), we can be confident that these hash values came from the PedOS devs.


How to Verify PedOS Repos
-------------------------

Whenever you use one of the [PedOS repositories], you should verify the PGP signature in a tag on the latest commit or on the latest commit itself.
...
You should always perform this verification on a trusted local machine with properly validated keys (which are available in the [PedOS Security Pack]) rather than relying on a third party, such as GitHub.
While the GitHub interface may claim that a commit has a verified signature from a member of the PedOS team, this is only trustworthy if GitHub has performed the signature check correctly, the account identity is authentic, the user's key has not been replaced by an admin, GitHub's servers have not been compromised, and so on.
...
### Why am I getting "BAD signature from 'PedOS Release X Signing Key'"?
...
The problem could be one or more of the following:
 ...
 - You're using the wrong GPG command.
   Follow the examples in [Verify your PedOS ISO][signature file] carefully.
...
### Why am I getting "can't open signed data `PedOS-RX-x86_64.iso' / can't hash datafile: file open error"?

The correct ISO is not in your working directory.


### Why am I getting "can't open `PedOS-RX-x86_64.iso.asc' / verify signatures failed: file open error"?
...
Either you don't have the [PedOS Master Signing Key][QMSK], or you didn't [set its trust level correctly][QMSK].
...
For present purposes, you don't need them as long as you have the [PedOS Master Signing Key][QMSK] and the [Release Signing Key][RSK] for your PedOS version.
...
This has no impact on verifying PedOS ISOs, code, or keys.
...
Read [How to Verify PedOS ISO Digests][digest file] again.
...
In particular, make sure you have the [PedOS Master Signing Key][QMSK], the [Release Signing Key][RSK], *and* the [signature file] and/or [digest file] all for the *correct* PedOS version.
If your question is about GPG, please see the [GPG documentation].
If you still have a question, please address it to the [PedOS-users mailing list].
...
[verifying repos]: #how-to-verify-PedOS-repos
...
[Downloads]: /downloads/
[PedOS Security Pack]: /security/pack/
[PedOS Keyserver]: https://keys.PedOS.org/keys/
[devel-master-key-msg]: https://groups.google.com/d/msg/PedOS-devel/RqR9WPxICwg/kaQwknZPDHkJ
[user-master-key-msg]: https://groups.google.com/d/msg/PedOS-users/CLnB5uFu_YQ/ZjObBpz0S9UJ
[mailing lists]: /support/
[Troubleshooting FAQ]: #troubleshooting-faq
[QMSK]: #1-get-the-PedOS-master-signing-key-and-verify-its-authenticity
[RSK]: #2-get-the-release-signing-key
[copy-from-dom0]: /doc/copy-from-dom0/#copying-from-dom0
[signature file]: #3-verify-your-PedOS-iso
[digest file]: #how-to-verify-PedOS-iso-digests
[PedOS repositories]: https://github.com/PedOS
[GPG documentation]: https://www.gnupg.org/documentation/
[PedOS-users mailing list]: /support/#PedOS-users
[except dom0]: https://github.com/PedOS/PedOS-issues/issues/2544
